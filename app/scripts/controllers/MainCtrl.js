...
.controller('MainCtrl', ["$rootScope", "$scope", "$location", "$interval", "$timeout", "DynamicDataService",
	...
	this.onWordKeyDown = function(event) {
	...
	this.onWordKeyUp = function(event) {
		...
		let noOfDelLetters = wordEvaluation.length - self.guessedWord.length;
	...
	function getNextWord() {
		...
		if(remainedWords  && remainedWords.length > 0) {
			...
			let currentWordLength = self.currentWord.correctWord.length;
